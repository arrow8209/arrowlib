...
protected:
    ...
    typedef TaskOneThread_TimerPrecision3<T, TimerPrecision> Local;
...
protected:
    ...
    bool AddTask(TRunFun pRunFun, TClearCacheFun pClearCacheFun, Args... args)
    {
        ...
        std::unique_lock<std::mutex> lock(m_mutexIsEmpty);
        m_cvIsEmpty.notify_one();
...
private:
    ...
    void RunThread()
    {
        ...
        while (m_bIsRun)
        {
            ...
            {
                std::unique_lock<std::mutex> lock(m_mutexIsEmpty);
                if (m_cvIsEmpty.wait_for(lock, m_msTimerPrecision, [this]()
                                         { return !m_QueueTask.Empty(); }) == false)
                {
                    RunThreadTimer();
                    continue;
                }
...
private:
    ...
    std::mutex m_mutexIsEmpty;
    std::condition_variable m_cvIsEmpty;
...
template<typename T>
using TaskOneThread3 = TaskOneThread_TimerPrecision3<T, 1>;
