...
public:
    ...
    ~CRunTime()
    {
        ...
        {
            std::cout << "Time-" << nIndex << ":" << (*it2 - *it1) * 1000 / CLOCKS_PER_SEC << "ms ";
        }
        std::cout << " Time-" << nIndex << ":" << (std::clock() - *it1) * 1000 / CLOCKS_PER_SEC << "ms ";
        std::cout << "All Time:" << (std::clock() - *m_ListClock.begin()) * 1000 / CLOCKS_PER_SEC << "ms " << std::endl;
    }
private:
